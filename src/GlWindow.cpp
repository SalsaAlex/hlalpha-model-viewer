...
// Purpose:
//-----------------------------------------------------------------------------
GlWindow::GlWindow( mxWindow * parent, int x, int y, int w, int h,
...
int GlWindow::handleEvent( mxEvent * event )
{
	...
	case mxEvent::Idle:
	{
		...
		if ( !g_bStopPlaying )
			g_studioRender.AdvanceFrame( ( curr - prev ) * g_viewerSettings.speedScale );
	...
	case mxEvent::KeyDown:
	{
		...
		case 32:
		{
			int iSeq = g_studioRender.GetSequence();
			if ( iSeq == g_studioRender.SetSequence( iSeq + 1 ) )
			{
				g_studioRender.SetSequence( 0 );
			}
		}
		break;
...
int GlWindow::loadTexture( const char * filename, int name )
{
	if ( !filename || !strlen( filename ) )
	{
		if ( d_textureNames[name] )
		{
			...
			if ( name == 0 )
				strlcpy( g_viewerSettings.backgroundTexFile, "", sizeof( g_viewerSettings.groundTexFile ) );
			else
				strlcpy( g_viewerSettings.groundTexFile, "", sizeof( g_viewerSettings.groundTexFile ) );
		}
	...
	{
		if ( name == 0 )
			strlcpy( g_viewerSettings.backgroundTexFile, filename, sizeof( g_viewerSettings.backgroundTexFile ) );
		else
			strlcpy( g_viewerSettings.groundTexFile, filename, sizeof( g_viewerSettings.backgroundTexFile ) );
		...
		if ( image->bpp == 8 )
		{
			...
			g_studioRender.UploadTexture( &texture, ( byte * ) image->data, ( byte * ) image->palette, name + 1 );
		}
		else
		{
